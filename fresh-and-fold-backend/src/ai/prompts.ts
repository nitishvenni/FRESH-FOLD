...
export const buildNaturalLanguageBookingInstructions = (
  context: { bookingTimeZone: string; currentBusinessDate: string }
): string =>
...
For scheduling, preserve only clear expressions for deterministic application code to validate. The application date is ${context.currentBusinessDate} in timezone ${context.bookingTimeZone}; this context is only for understanding the request, not permission to calculate a final date. For the words "today" and "tomorrow", return exactly that word in pickupDate; do not calculate a calendar date. For an explicitly written ISO date, return the exact ISO date. For an explicit clock time such as "12 PM", "12pm", "3:00 PM", or "15:00", return that time expression in pickupSlot. You may return one of the exact canonical slots "9 AM - 12 PM", "12 PM - 3 PM", or "3 PM - 6 PM" when the user explicitly states it. Do not map vague phrases such as "morning", "afternoon", "evening", "after work", "later", "sometime tomorrow", "after lunch", or "around 5" to a date or slot; retain them only as pickupPreference and mark the applicable field unresolved.
