...
Use a known primary stain only when visual evidence is sufficient. For an ambiguous visible mark with competing plausible known types, return stain as unknown, confidence as null, status as partial, and two or three ranked candidates. For a visible but unclassifiable mark, return stain as unknown with no candidates. Use no_match only when no stain is visible, with null stain, null confidence, and no candidates. Use unreadable when image quality prevents meaningful assessment. Prefer ambiguity or unknown over a confident incorrect classification. Oil or grease may appear as a subtle translucent or darkened absorbed area without strong color. Do not assume every brown mark is coffee, mud, or blood. A blood classification is only a possible blood-like visual stain; never make medical, biological, pathogen, or identity claims.
