  ...
  return (
    <View style={[styles.screen, { backgroundColor: theme.background, paddingTop: insets.top + 18 }]}>
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 32 }]}>
        ...
        <Text style={[styles.title, { color: theme.text }]}>Care Label Reader</Text>
        <Text style={[styles.subtitle, { color: theme.textMuted }]}>Take or choose one clear photo of a garment care label. You will review advisory instructions.</Text>
        {previewUri ? <Image accessibilityLabel="Selected care-label preview" source={{ uri: previewUri }} style={[styles.preview, { borderColor: theme.border }]} /> : null}

        {processing ? (
          <Card style={[styles.processingCard, { backgroundColor: theme.surfaceElevated, borderColor: theme.border }]}>
            <ActivityIndicator size="large" color={theme.primary} />
            <Text style={[styles.processingTitle, { color: theme.text }]}>Reading care label</Text>
            <Text style={[styles.processingCopy, { color: theme.textMuted }]}>Extracting visible label evidence. This does not create a booking.</Text>
        ...
        {error ? (
          <Card style={[styles.errorCard, { backgroundColor: theme.surfaceElevated, borderColor: theme.border }]}>
            ...
            {error.retryable ? <TouchableOpacity accessibilityRole="button" style={[styles.secondaryButton, { borderColor: theme.border }]} onPress={() => setError(null)}><Text style={[styles.secondaryButtonText, { color: theme.text }]}>Try another photo</Text></TouchableOpacity> : null}
          </Card>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  back: { flexDirection: "row", alignItems: "center", alignSelf: "flex-start", minHeight: 40 }, backText: { marginLeft: 6, fontSize: 15, fontWeight: "600" },
  title: { marginTop: 18, fontSize: 30, fontWeight: "700" }, subtitle: { marginTop: 8, fontSize: 15, lineHeight: 22 },
  preview: { width: "100%", height: 240, marginTop: 24, borderWidth: 1, borderRadius: 20, resizeMode: "cover" }, actions: { marginTop: 28, gap: 12 },
  primaryButton: { minHeight: 54, borderRadius: 16, flexDirection: "row", alignItems: "center", justifyContent: "center" }, primaryButtonText: { marginLeft: 9, color: "#FFFFFF", fontSize: 16, fontWeight: "700" },
  secondaryButton: { minHeight: 52, borderRadius: 16, borderWidth: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingHorizontal: 16 }, secondaryButtonText: { marginLeft: 8, fontSize: 15, fontWeight: "600" },
  processingCard: { marginTop: 28, alignItems: "center", paddingVertical: 30 }, processingTitle: { marginTop: 16, fontSize: 18, fontWeight: "700" }, processingCopy: { marginTop: 8, fontSize: 14, lineHeight: 20, textAlign: "center" },
  errorCard: { marginTop: 22, alignItems: "center" }, errorTitle: { marginTop: 9, fontSize: 16, fontWeight: "700", textAlign: "center" }, errorCopy: { marginTop: 6, fontSize: 14, lineHeight: 20, textAlign: "center" }, diagnosticCopy: { marginTop: 8, fontSize: 12, lineHeight: 18, textAlign: "center" },
  manualLink: { alignSelf: "center", minHeight: 48, justifyContent: "center", marginTop: 22 }, manualLinkText: { fontSize: 15, fontWeight: "700" },
});
