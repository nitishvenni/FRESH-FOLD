  ...
  return (
    <View style={[styles.screen, { backgroundColor: theme.background, paddingTop: insets.top + 18 }]}>
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 32 }]}>
        ...
        <Text style={[styles.subtitle, { color: theme.textMuted }]}>Take or choose one clear photo of your garments. You will review every result.</Text>

        {previewUri ? (
          <Image accessibilityLabel="Selected garment preview" source={{ uri: previewUri }} style={[styles.preview, { borderColor: theme.border }]} />
        ) : null}

        {processing ? (
          <Card style={[styles.processingCard, { backgroundColor: theme.surfaceElevated, borderColor: theme.border }]}>
            <ActivityIndicator size="large" color={theme.primary} />
            <Text style={[styles.processingTitle, { color: theme.text }]}>Analyzing your garments</Text>
            <Text style={[styles.processingCopy, { color: theme.textMuted }]}>Preparing a reviewable result. This does not create a booking.</Text>
