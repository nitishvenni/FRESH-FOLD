  ...
  return (
    <View style={[styles.screen, { backgroundColor: theme.background, paddingTop: insets.top + 18 }]}>
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 32 }]}>
        ...
        <Text style={[styles.title, { color: theme.text }]}>Stain Detection</Text>
        <Text style={[styles.subtitle, { color: theme.textMuted }]}>Take or choose one clear photo of a visible mark or stain. You will review advisory guidance.</Text>

        {previewUri ? <Image accessibilityLabel="Selected stain preview" source={{ uri: previewUri }} style={[styles.preview, { borderColor: theme.border }]} /> : null}

        {processing ? (
          <Card style={[styles.processingCard, { backgroundColor: theme.surfaceElevated, borderColor: theme.border }]}>
            <ActivityIndicator size="large" color={theme.primary} />
            <Text style={[styles.processingTitle, { color: theme.text }]}>Analyzing stain</Text>
            <Text style={[styles.processingCopy, { color: theme.textMuted }]}>Preparing cautious care guidance. This does not create a booking.</Text>
        ...
        {error ? (
          <Card style={[styles.errorCard, { backgroundColor: theme.surfaceElevated, borderColor: theme.border }]}>
            ...
            {isDevelopmentBuild && error.diagnostic ? (
              <Text style={[styles.diagnosticCopy, { color: theme.textMuted }]}>Code: {error.diagnostic.code}{typeof error.diagnostic.status === "number" ? ` • HTTP ${error.diagnostic.status}` : ""}{error.diagnostic.requestId ? ` • Request: ${error.diagnostic.requestId}` : ""}</Text>
            ) : null}
            {error.retryable ? (
